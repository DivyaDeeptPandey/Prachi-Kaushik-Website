...
const NewsGallery = () => {
  ...
  const categories = [
    ...
    { id: 'menstrual', name: 'Menstrual Health & Hygiene Programme' },
    { id: 'entrepreneurship', name: 'Women Entrepreneurship Development' },
    { id: 'community', name: 'Community Development' },
    /*{ id: 'youth', name: 'Youth Empowerment' },
    { id: 'violence', name: 'Voice against violence' },
    { id: 'advocacy', name: 'Advocacy and policy changes' }*/
  ];
  ...
  const categorizedNews = activeCategory === 'all'
    ? newsItems
    : newsItems.filter(news => news.category === activeCategory);

  // Pagination setup
  const itemsPerPage = 6;
  const totalPages = Math.ceil(categorizedNews.length / itemsPerPage);
  const indexOfLast = currentPage * itemsPerPage;
  const indexOfFirst = indexOfLast - itemsPerPage;
  const currentItems = categorizedNews.slice(indexOfFirst, indexOfLast);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Mobile Menu Button */}
      <button
        className="lg:hidden fixed top-4 left-4 z-50 bg-primary text-white p-3 rounded-md shadow-lg"
      ...
      <div className="flex">
        {/* ==============================
      Desktop Sidebar (Commented Out)
  ============================== */}
        {/*}
        <div className="hidden lg:block w-64 bg-white border-r border-gray-200 h-screen sticky top-0 shadow-md overflow-y-auto">
          <div className="p-6">
            <h2 className="satoshi-bold text-xl text-gray-900 mb-6 border-b pb-3">News Categories</h2>
              ...
              {categories.map((category) => (
                <li key={category.id}>
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 rounded-lg transition-all duration-200 satoshi-medium ${
          ...
          {isMobileMenuOpen && (
            <>
              ...
              <motion.div
                ...
                className="fixed inset-y-0 left-0 w-64 bg-white z-50 lg:hidden shadow-lg"
              >
                <div className="p-6 h-full overflow-y-auto">
                  <div className="flex justify-between items-center mb-6">
                    <h2 className="satoshi-bold text-xl text-gray-900">News Categories</h2>
                    <button
                      onClick={() => setIsMobileMenuOpen(false)}
        ...
        */}
        ...
        <div className="flex-1 min-h-screen">
          ...
          <div className="bg-white border-b border-gray-200 p-6 lg:p-8">
            <h1 className="satoshi-bold text-2xl lg:text-3xl text-gray-900">
              {categories.find(cat => cat.id === activeCategory)?.name || 'News Articles'}
            </h1>
            <p className="satoshi-regular text-gray-600 mt-2">
              {categorizedNews.length} article{categorizedNews.length !== 1 ? 's' : ''} found
            </p>
          </div>
          ...
          <div className="p-6 lg:p-8">
            ...
            {categorizedNews.length === 0 && (
              <div className="text-center py-12">
                ...
                <h3 className="satoshi-medium text-lg text-gray-900 mb-2">No articles found</h3>
                <p className="satoshi-regular text-gray-600">Try selecting a different category</p>
              </div>
            )}
            ...
            {totalPages > 1 && (
              <div className="flex justify-center items-center mt-10 space-x-2">
                <button
                  ...
                  className="px-3 py-2 rounded-md border text-sm satoshi-medium disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-100"
                >
                  Prev
                </button>
                {[...Array(totalPages)].map((_, i) => (
                  <button
                    ...
                    className={`px-3 py-2 rounded-md border text-sm satoshi-medium transition-colors ${currentPage === i + 1
